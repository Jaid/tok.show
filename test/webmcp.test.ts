...
const originalState = {
  ...
  inputTabs: state.inputTabs.map(tab => ({
    ...tab,
    binaryData: tab.binaryData?.slice() ?? null,
  })),
...
afterEach(() => {
  ...
  state.inputTabs = originalState.inputTabs.map(tab => ({
    ...tab,
    binaryData: tab.binaryData?.slice() ?? null,
  }))
...
describe('WebMCP', () => {
  ...
  test('GUI tools edit and inspect the current editor', async () => {
    ...
    const editor = await readEditor.execute({}, executeOptions) as {
      text: string
      type: string
    }
    const ui = await inspect.execute({}, executeOptions) as {editor: {activeTab: {
      characters: number
      type: string
    }}}
    expect(editor).toMatchObject({
      type: 'text',
      text: 'WebMCP GUI state',
    })
    expect(ui.editor.activeTab).toMatchObject({
      type: 'text',
      characters: 16,
    })
  ...
  test('headless tools accept arrayable models without changing GUI state', async () => {
    ...
    const counts = await countTokens.execute({
      text: 'headless tokenizer',
      models: 'glm',
    }, executeOptions) as {counts: {glm: number}}
    const tokens = await tokenize.execute({
      text: 'headless tokenizer',
      models: 'glm',
    }, executeOptions) as {tokens: {glm: Array<number>}}
    const spans = await createSpans.execute({
      text: 'headless tokenizer',
      models: ['glm'],
    }, executeOptions) as {spans: {glm: Array<{
      end: number
      start: number
    }>}}
  ...
  test('rejects undeclared properties in code instead of relying on schema enforcement', async () => {
    ...
    await expect(countTokens.execute({
      text: 'strict',
      models: 'glm',
      unexpected: true,
    } as any, executeOptions)).rejects.toThrow('unknown property “unexpected”')
  ...
  test('ContentSource URL inputs are fetched and text/url are mutually exclusive', async () => {
    ...
    try {
      const result = await countTokens.execute({
      ...
      await expect(countTokens.execute({
        ...
        url: 'https://example.test/content',
        models: 'glm',
      }, executeOptions)).rejects.toThrow('Exactly one of text or url')
    } finally {
  ...
  test('create_spans identifies normalized tokenizer input', async () => {
    const {tools} = createTools()
    const createSpans = getTool(tools, 'create_spans')
    const result = await createSpans.execute({
      text: '  HELLO  WORLD  ',
      models: 'sdxl',
    }, executeOptions) as {
      spanInputs: {sdxl: {
    ...
    expect(result.spanInputs.sdxl).toEqual({
      type: 'processed',
      text: 'hello world',
    })
    expect(result.spans.sdxl).toEqual([{
      start: 0,
      end: 6,
    }, {
      start: 6,
  ...
  test('compare and create_permalink operate headlessly', async () => {
    ...
    const comparison = await compare.execute({
      ...
      cases: Array<{tokens: number}>
      model: string
      summary: {spread: number}
    }
